...
function onMouseDown(event) {
    ...
    circles.push(
        createRandomCircle(
            Math.floor(Math.random() * 300) + 100,
            keysSounds[Object.keys(keysSounds)[Math.floor(Math.random() * 26)]]
                .color
        )
    );
    ...
    keysSounds[
        Object.keys(keysSounds)[Math.floor(Math.random() * 26)]
    ].sound.play();
}
...
function onFrame(event) {
    for (var i = 0; i < circles.length; i++) {
        ...
        if (circles[i].area < 10) {
            // remove circles after they've become small enough
